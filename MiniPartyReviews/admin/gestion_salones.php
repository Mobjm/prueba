...
if (isset($_POST['agregar'])) {
    ...
    $linkmaps = trim($_POST['linkmaps']);

    if ($nombre && $direccion && $idzona) {
        $stmt = $conn->prepare("INSERT INTO Salones (IdZona, Nombre, Direccion, Telefono, LinkMaps, Descripcion) 
                                VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$idzona, $nombre, $direccion, $telefono, $linkmaps, $descripcion]);
...
if (isset($_POST['actualizar'])) {
    ...
    $stmt = $conn->prepare("UPDATE Salones 
                            SET IdZona=?, Nombre=?, Direccion=?, Telefono=?, LinkMaps=?, Descripcion=? 
                            WHERE IdSalon=?");
    $stmt->execute([$idzona, $nombre, $direccion, $telefono, $linkmaps, $descripcion, $id]);
    $mensaje = "✅ Salón actualizado correctamente.";
}

// Listar salones
$stmt = $conn->prepare("SELECT s.IdSalon, s.Nombre, s.Direccion, s.Telefono, s.LinkMaps, s.Descripcion, z.Nombre AS ZonaNombre 
                        FROM Salones s 
...
<?php if($salonEditar): ?>
    ...
    <form method="post">
        ...
        <input type="text" name="linkmaps" value="<?= htmlspecialchars($salonEditar['LinkMaps']) ?>">
...
<?php else: ?>
    ...
    <form method="post">
        ...
        <label>Link de Google Maps:</label>
        <input type="text" name="linkmaps" placeholder="https://maps.google.com/...">
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <?php if($salon['LinkMaps']): ?>
                    <a href="<?= htmlspecialchars($salon['LinkMaps']) ?>" target="_blank">🌍 Maps</a>
                <?php else: ?>
                    —
                <?php endif; ?>
            </td>
